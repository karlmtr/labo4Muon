...
//	csock_connect
////////////////////////////////////////////
SOCKET csock_connect(const char *ipAddress, int port)
{
	SOCKET sck = 0;
...
//	csock_udpconnect
////////////////////////////////////////////
SOCKET csock_udpconnect(const char *ipAddress, int port, struct sockaddr_in *dest)
{
	SOCKET sck = 0;
	struct sockaddr_in target_address;
...
//	CROPEN
////////////////////////////////////////////
short CROPEN(const char *address)
{
#ifdef WIN32
	WSADATA data;
	int ret;
...
#else
		if ((int)crate_info[crate_id].irq_tid >= 0){
...
short BLKBUFFS(short crate_id, short value)
{
	...
	len = strlen(CRATE_TxBuffer);
	for(i = len+1; i < 256; i++) CRATE_TxBuffer[i] = 0;
...
short BLKTRANSF(short crate_id, BLK_TRANSF_INFO *blk_info, unsigned int *buffer)
{
	...
	switch (blk_info->F >> 3) {
		...
		case 2: //Write Operation
			...
			for (i = 0; i < rows; i++) {
				if (i == (rows - 1))
					sprintf(blk_ascii_buf, "%03d", blk_info->totsize - i*blk_info->blksize);
				else
